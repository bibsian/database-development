...
# File to edit
filename = "sev178_grazingexclospp_04102009.txt"
...
for (i in 1:nrow(spplist)){
  for(j in 1:length(taxa_headers)){
    data[which(
      trimws(as.character(data$Code)) == trimws(
...
unique(data$Month)
data$month_num <- rep(NA, nrow(data))
data$month_num[which(as.character(data$Month) == 'May')] <- 05
data$month_num[which(as.character(data$Month) == 'April')] <- 04
data$month_num[which(as.character(data$Month) == 'September')] <- 09
...
data$site_plot <- paste0(data$Site, data$Plot)
