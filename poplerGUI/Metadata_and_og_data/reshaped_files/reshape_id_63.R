...
adm.req <-c("dplyr", "tidyr", "stringr", "plyr")
...
# File to edit
filename = "SGS_LTER_LongTermWebVegetation_LTMntrVegWebsCov.txt"
spp = "SGS_plant_species_list_updated.csv"
# Path to files
...
taxa_headers <- c(
  'Taxa_kingdom', 'Taxa_order', 'Taxa_family', 'Taxa_genus', 'Taxa_species')
# Adding headers to 
...
for (i in 1:nrow(spplist)){
  for(j in 1:length(taxa_headers)){
    data[which(
      tolower(trimws(as.character(data$SPECIES))) == tolower(trimws(
        as.character(spplist$LTER.SpeciesCode)))[i]), 
