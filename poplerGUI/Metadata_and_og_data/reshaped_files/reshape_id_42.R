...
# File to edit
filename = "sev212_pdogvegcover_20130509.txt"
...
data[which(data==-888, arr.ind = T)] <- -99999

data$DATE <- as.character(data$DATE)
data$DATE[which(data$DATE == '.' & data$SEASON == 'F')] <- '10/01/2006'
data$DATE[which(data$DATE == '.' & data$SEASON == 'S')] <- '04/01/2006'
...
#spplist$spp_id
data$treatment <- do.call(rbind, str_split(as.character(data$PLOT), '^[A-z]{1}'))[,2]
data$plot_name <- do.call(rbind, str_split(as.character(data$PLOT), '[A-z]{1}$'))[,1]
