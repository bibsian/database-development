...
# File to edit
filename = "sev215_bosquebees_20140128.txt"
spp = "SEV_bee_species_list.txt"
# Path to file
...
data$Taxa_phylum <- 'Arthropoda'
data$Taxa_class <- 'Insecta'
data$Taxa_order <- 'Hymenoptera'
data$Taxa_family <- NA
data$Taxa_genus <- NA
data$Taxa_species <- NA


data$updated_bee_species<- do.call(
  rbind, str_split(as.character(data$Bee_Species), '[F]$'))[,1]

for (i in 1:length(keys)){

  data$Taxa_family[c(which(as.character(data$updated_bee_species) == keys[i]))] <- 
    as.character(spplist$FAMILY)[i]
  data$Taxa_genus[c(which(as.character(data$updated_bee_species) == keys[i]))] <- 
    as.character(spplist$GENUS)[i]
  data$Taxa_species[c(which(as.character(data$updated_bee_species) == keys[i]))] <- 
    as.character(spplist$SPECIES)[i]
  
}


data$site <- substring(as.character(data$SiteRepSeason), 1, 1) 
data$rep <- substring(as.character(data$SiteRepSeason), 2, 2) 
data$season <- substring(as.character(data$SiteRepSeason), 3, 4) 
data$year <- paste0('20',substring(as.character(data$SiteRepSeason), 5))
