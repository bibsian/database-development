...
# File to edit
filename = "sev097_smesquad_20160216_0.txt"
...
data$year <- do.call(rbind, str_split(data$Season, '^[A-Z]{1,}'))[,2]
data$seas <- do.call(rbind, str_split(data$Season, '[0-9]{3,}'))[,1]

data$record_date[
  which(data$record_date == '' & data$seas == 'SPRING')]

data$updated_date <- as.character(data$record_date)

data$updated_date[
  which(data$record_date == '' & data$seas == 'SPRING')] <- 
  paste0('04/01/', data$year[
    which(data$record_date == '' & data$seas == 'SPRING')])
    
data$Treatment[which(as.character(data$Treatment) == 'C')]
data$trt_updated <- as.character(data$Treatment)
data$trt_updated[which(as.character(data$Treatment) == 'C')]<- 'Ctrl'
