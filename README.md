...
##raw_climate: Table of orginal (or aggregated) meteorlogical data from individual LTER stations 
##climate_site: Table of meteorlogical sites wiht a given LTER station
##lter_table: Table of all LTER stations and site abbreviations (w/ lat & long)
##site_table: Table of designated sites within a study that gathered data at a LTER station (w/ lat & log if available)
##main_table: Table of metadata that describes a study (includes temporal information, levels of spatial replication, PI's, contact emails, community data set indicator, etc). See the 'db' folder for a complete overview of fields.
##taxa_table: Table of unique instances of a record regarding the taxonomy of study organisms
##raw_table: Table of the orginal observations gathered from the study.
