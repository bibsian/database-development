...
metarecords_stmt = conn.execute(select([Maintable.metarecordid]).distinct())
metarecords_df = DataFrame(metarecords_stmt.fetchall())
metarecords_list = metarecords_df[0].values.tolist()
metarecords_list.sort()

metarecords_list = [1,2,3]
for z in metarecords_list:
    ...
    proj_df_before_convert = proj_df.copy()
    try:
